...
static inline uint16_t read_adc() {
    ...
    // Configuración para single-shot AIN0 vs GND ±4.096V 1600SPS
    ...
    // Esperar a que la conversión termine, con timeout
    int timeout = 1000; // max ~100ms
    while(timeout--) {
    ...
    uint16_t value = ((raw << 8) | (raw >> 8)) >> 4; // 12 bits
    value &= 0x0FFF;
    ...
    for (int i = 0; i < ROW_SIZE; ++i) {
        ...
        for (int j = 0; j < COL_SIZE; ++j) {
            ...
            if(val > 4000){
                ...
                val=val*35;// Escalar como python;
            }
            matrix[i][j] = val;
        }
    }
